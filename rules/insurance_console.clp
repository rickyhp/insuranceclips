...
(defrule citizenship=SC_SPR
	(age ?)
    (not (citizenship ?))
=>	(printout t crlf "What is your citizenship? (s)ingaporean/(p)r/(f)oreigner" crlf)
	(bind ?answer (read))
...
=>	
	...
	(if (eq ?response y)
        ...
        else(assert (current_goal (goal Critical-Care-Advantage) (cf -1.0)))			
    )
	
	(if (eq ?g f)
		then(if (eq ?a 2)
			then(assert (flexi-maternity-cover start))
			else(if (eq ?a 3)
					then(assert (flexi-maternity-cover start))
				)
			)		
		else(assert (income-protection start))
	)	
)
...
;;; Based on your age, we recommend you to get financial protection against severe disability, press (y) to continue, (n) to skip this plan
(defrule longterm-protection-qn
	(long-term-care start)    
	(not (longterm-protection-ans ?))
=>	(printout t "Based on your age, we recommend you to get financial protection against severe disability, press (y) to continue, (n) to skip this plan" crlf)
	(bind ?answer (read))
	(assert (longterm-protection-ans ?answer))	
)
...
(defrule conclusion-ElderShield
   ...
   => 
	...
	(printout t crlf "We recommend you below GE long term care package :")
	(printout t crlf "Basic ElderShield               : " (/ (+ (- 1.0 ?cf-adl) (- 1.0 ?cf-year) (- 1.0 ?cf-amount)) 3))
	(printout t crlf "ElderShield Comprehensive 3 ADLs: " (/ (+ (- 1.0 ?cf-adl) (+ 0.0 ?cf-year) (+ 0.0 ?cf-amount)) 3))
	(printout t crlf "ElderShield Comprehensive 2 ADLs: " (/ (+ (+ 0.0 ?cf-adl) (+ 0.0 ?cf-year) (+ 0.0 ?cf-amount)) 3) crlf)	
)
...
(defrule pregnant-qn
    ...
    (gender f)
    (or (age 2) (age 3))
=> (printout t crlf "Are you pregnant? (y)es/(n)o" crlf)
   (bind ?response (read))
   (if (eq ?response n)
...
=>
  (if(> ?cf-Flexi-Maternity-Cover-Enhanced 0)
	then(printout t crlf "Flexi-Maternity-Cover-Enhanced: " ?cf-Flexi-Maternity-Cover-Enhanced crlf)
		(assert (recommendation (Flexi-Maternity-Cover-Enhanced ?cf-Flexi-Maternity-Cover-Enhanced)))
  )
  (if(> ?cf-Flexi-Maternity-Cover-Essential 0)	
	then(printout t crlf "Flexi-Maternity-Cover-Essential: " ?cf-Flexi-Maternity-Cover-Essential crlf)
		(assert (recommendation (Flexi-Maternity-Cover-Essential ?cf-Flexi-Maternity-Cover-Essential)))
  )
...
;;; Do you like to get your current income protection?
(defrule income-protection-qn
    (income-protection start)
	(or (employment-status f)(employment-status p))
    (not (fixed-vs-flexible-ans ?))
=>	(printout t "Do you like to get your current income protection? (y)es/(n)o" crlf)
	(bind ?answer (read))
	(assert (income-protection-ans ?answer))
)

;;; Do you prefer fixed or flexible payout?
(defrule fixed-vs-flexible-qn
    (income-protection-ans y)
	(or (employment-status f)(employment-status p))
    (not (fixed-vs-flexible-ans ?))
